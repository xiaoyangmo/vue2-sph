  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in userAddressList" :key="address.id">
        ...
        <p @click="changeAddress(address,userAddressList)">
      ...
      <div class="detail">
        <h5>商品清单</h5>
        <ul class="list clearFix" v-for="list in detailArrayList" :key="list.id">
          <li>
...
<script>
  ...
  export default {
    ...
    computed:{
      ...mapState('trade',['userAddressList','detailArrayList','tradeList']),
      address(){
        return this.userAddressList.find(item=>item.isDefault==='1')||{}
      }
    },
    methods:{
      changeAddress(address,list){
        list.forEach(item=>{
          item.isDefault='0'
        });
        address.isDefault='1';
      ...
      submit(){
        ...
        let result=this.$store.dispatch('trade/submitOrder',{tradeNo,data});
        result.then(data=> this.$router.push({path:'/pay', query: {orderId:data}})).catch(error=>console.log(error))
      }
    },
    mounted() {
